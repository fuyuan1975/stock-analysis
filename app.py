    ...
    if analyze_button or st.session_state.get('analyzed', False):
        ...
        try:
            ...
            with st.spinner(f'正在分析 {symbol}...'):
                ...
                analyzer.stock.history(period=period)
                analyzer.hist = analyzer.stock.history(period=period)
            ...
            with metrics_cols[0]:
                try:
                    if len(analyzer.hist) > 0:
                        ...
                        prev_close = analyzer.hist['Close'].iloc[-2] if len(analyzer.hist) > 1 else current_price
            ...
            with metrics_cols[4]:
                try:
                    dividend_yield = ratios.get('股息率', 0)
                    if dividend_yield > 0:
